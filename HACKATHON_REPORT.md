...
* **TTS Voice Play, Pause, Resume & Stop Controls**: Integrates HTML5 Speech Synthesis. Employs a 60ms delay queue buffer and matching ref validations to eliminate race conditions, offering rock-solid inline `<Play />`, `<Pause />`, and `<VolumeX />` controls.
* **Multi-Chat Session Manager**: Features a slide-out sidebar to save, load, and delete conversation logs. Includes automated query auto-naming and localStorage state partitioning that separates guest chat logs from user account histories.
...
* **WebRTC Camera Capture Modal & Laptop Fallbacks**: Fully integrated split media upload dropzones (Choose File / Use Camera) across Adopter Dashboard, Behavior Hub, and Global Floating Chat. Captures live images directly in-page using a responsive modal backed by `getUserMedia` video streams and canvas rendering. Includes device check catch fallbacks (reverting to `{ video: true }` if environment camera constraints are missing) to run smoothly on standard laptop/desktop webcams.
* **Split Upload Preview Separation**: Tracks file upload sources to prevent duplicate indicators, isolating filename display labels only to the specific button (library select vs webcam shot) that was clicked.
